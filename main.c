...
int main(int argc, char *argv[], char *envp[])
{
  ...
  int mutt_init(struct ConfigSet *cs, const char *dlevel, const char *dfile, bool skip_sys_rc, struct ListHead *commands)
  {
    ...
    int execute_commands(struct ListHead *p)
    {
      int rc = 0;
      struct Buffer *err = buf_pool_get();
    ...
    bool get_hostname(struct ConfigSet *cs)
    {
  ...
  if (dump_variables || !STAILQ_EMPTY(&queries))
  {
    ...
    {
      int get_elem_queries(struct ListHead *queries, struct HashElemArray *hea)
      {
        int rc = 0;
        struct ListNode *np = NULL;
